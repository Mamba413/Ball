...
def get_screened_var(d, *bcor_stat):
    ind = []
    bcor_stat = list(map(np.array, bcor_stat))
    num_groups = len(bcor_stat)
    for i in range(d):
        for stat in bcor_stat:
            ind.append(np.argmax(stat)+1)
            stat[ind[i]-1] = -1.0
    if num_groups > 1:
        ind3 = [ind[i] for i in range(0, len(ind), 2)]
        ind4 = [ind[i] for i in range(1, len(ind), 2)]
#        ind = [i for i in ind1 if i in ind2]
    return ind
